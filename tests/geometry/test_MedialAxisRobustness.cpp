...
TEST_F(MedialAxisRobustnessTest, DegenerateTriangle) {
  std::vector<Point2D> collinear = {
      {0, 0}, {1, 0}, {2, 0}  // All collinear - zero area polygon
  };

  // Collinear points form a zero-area polygon which would hang OpenVoronoi.
  ...
  EXPECT_TRUE(results.errorMessage.find("validation") != std::string::npos ||
              results.errorMessage.find("failed") != std::string::npos);
}
