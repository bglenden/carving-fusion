...
class Shape {
 public:
   ...
   */
  virtual void drawToSketch(Adapters::ISketch* sketch, Adapters::ILogger* logger = nullptr) const = 0;
 ...
 */
inline double calculateChordToArcError(const Point2D& start, const Point2D& end, const Point2D& center, double radius) {
  ...
  Point2D arcMid = center + Point2D(toChordMid.x / distToChordMid * radius, toChordMid.y / distToChordMid * radius);
