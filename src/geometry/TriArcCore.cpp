 ...
 * TriArcCore.cpp
 *
 * Core functionality for TriArc - constructor, basic getters, bounds, contains, centroid
...
bool TriArc::contains(const Point2D& point) const {
    ...
    // This is an approximation - a full implementation would need to handle curved edges
    ...
    double a = ((vertices_[1].y - vertices_[2].y) * (point.x - vertices_[2].x) +
                (vertices_[2].x - vertices_[1].x) * (point.y - vertices_[2].y)) / denom;
    double b = ((vertices_[2].y - vertices_[0].y) * (point.x - vertices_[2].x) +
                (vertices_[0].x - vertices_[2].x) * (point.y - vertices_[2].y)) / denom;
    double c = 1 - a - b;
