...
MedialAxisResults MedialAxisProcessor::computeMedialAxis(const Shape& shape) {
  ...
  log("This ensures geometry comes from actual Fusion profiles, not original "
      "shape parameters");
...
bool MedialAxisProcessor::validatePolygonForOpenVoronoi(const std::vector<Point2D>& polygon) {
  ...
  // Check if polygon is closed (last vertex should NOT equal first for our
  // algorithm) Fusion gives us properly closed polygons without duplicate
  // vertices
  ...
  if (lastEqualsFirst) {
    log("Warning: Last vertex equals first vertex - polygon appears to have "
        "duplicate "
        "closing vertex");
