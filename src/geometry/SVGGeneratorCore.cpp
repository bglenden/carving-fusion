...
SVGGenerator::SVGGenerator(double width, double height, double scale)
  ...
  svg_ << "<svg width=\"" << width_ << "\" height=\"" << height_ << "\" viewBox=\"0 0 " << width_ << " " << height_
...
void SVGGenerator::setBounds(const Point2D& min, const Point2D& max, double margin) {
...
void SVGGenerator::addPoint(const Point2D& point, const std::string& color, double radius, const std::string& label) {
  Point2D svg_point = worldToSVG(point);
  svg_ << "  <circle cx=\"" << svg_point.x << "\" cy=\"" << svg_point.y << "\" r=\"" << radius << "\" fill=\"" << color
       << "\"/>\n";
...
void SVGGenerator::addLine(const Point2D& start, const Point2D& end, const std::string& color, double strokeWidth,
                           const std::string& style) {
  Point2D start_svg = worldToSVG(start);
  Point2D end_svg = worldToSVG(end);

  svg_ << "  <line x1=\"" << start_svg.x << "\" y1=\"" << start_svg.y << "\" x2=\"" << end_svg.x << "\" y2=\""
       << end_svg.y << "\" stroke=\"" << color << "\" stroke-width=\"" << strokeWidth << "\"";
...
void SVGGenerator::addArc(const Point2D& center, double radius, double startAngle, double endAngle, bool anticlockwise,
                          const std::string& color, double strokeWidth) {
  double radius_svg = worldToSVG(radius);

  // Calculate start and end points
  Point2D start(center.x + radius * std::cos(startAngle), center.y + radius * std::sin(startAngle));
  Point2D end(center.x + radius * std::cos(endAngle), center.y + radius * std::sin(endAngle));
  ...
  svg_ << "  <path d=\"M " << start_svg.x << "," << start_svg.y << " A " << radius_svg << "," << radius_svg << " 0 "
       << (largeArc ? "1" : "0") << "," << (anticlockwise ? "0" : "1") << " " << end_svg.x << "," << end_svg.y
       << "\" stroke=\"" << color << "\" stroke-width=\"" << strokeWidth << "\" fill=\"none\"/>\n";
}

void SVGGenerator::addCircle(const Point2D& center, double radius, const std::string& color, double strokeWidth) {
  Point2D center_svg = worldToSVG(center);
  double radius_svg = worldToSVG(radius);

  svg_ << "  <circle cx=\"" << center_svg.x << "\" cy=\"" << center_svg.y << "\" r=\"" << radius_svg << "\" stroke=\""
       << color << "\" stroke-width=\"" << strokeWidth << "\" fill=\"none\"/>\n";
}

void SVGGenerator::addText(const Point2D& position, const std::string& text, const std::string& color,
                           double fontSize) {
  Point2D svg_pos = worldToSVG(position);
  svg_ << "  <text x=\"" << svg_pos.x << "\" y=\"" << svg_pos.y << "\" font-family=\"Arial, sans-serif\" font-size=\""
       << fontSize << "\" fill=\"" << color << "\" text-anchor=\"middle\">" << text << "</text>\n";
}
