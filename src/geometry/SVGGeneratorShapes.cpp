...
void SVGGenerator::addLeaf(const Leaf& leaf, const std::string& color, double strokeWidth) {
    if (!leaf.isValidGeometry()) {
        ...
        svg_ << "  <line x1=\"" << f1.x << "\" y1=\"" << f1.y << "\" x2=\"" << f2.x << "\" y2=\""
             << f2.y << "\" stroke=\"" << color << "\" stroke-width=\"" << strokeWidth
    ...
    // Note: f2_svg, center1_svg, center2_svg not currently used but may be needed for debugging
    ...
    svg_ << "A " << radius_svg << "," << radius_svg << " 0 0," << (arc1.anticlockwise ? "0" : "1")
         << " " << arcEnd1.x << "," << arcEnd1.y << " ";

    // Second arc from f2 back to f1
    Point2D arcEnd2 = worldToSVG(leaf.getFocus1());
    svg_ << "A " << radius_svg << "," << radius_svg << " 0 0," << (arc2.anticlockwise ? "0" : "1")
         << " " << arcEnd2.x << "," << arcEnd2.y;
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (triArc.isEdgeStraight(i)) {
            ...
            svg_ << "A " << radius_svg << "," << radius_svg << " 0 0,"
                 << (arc.anticlockwise ? "0" : "1") << " " << vNext_svg.x << "," << vNext_svg.y
                 << " ";
        }
    }
