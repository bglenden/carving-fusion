...
void TriArc::drawToSketch(Adapters::ISketch* sketch, Adapters::ILogger* logger) const {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (pointIndex < 0) {
            ...
            // if (logger) {
            //     logger->logError("Failed to add vertex point " + std::to_string(i));
            // }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (isStraight) {
            ...
            // For concave arcs (negative bulge), the arc curves toward the triangle center
            ...
            // For concave arcs, determine which direction curves toward triangle center
            Point2D triangleCenter = getCenter();
            Point2D toCenter =
                Point2D(triangleCenter.x - chordMid.x, triangleCenter.y - chordMid.y);

            // Choose perpendicular direction that points toward triangle center for concave arcs
            ...
            if (midIdx < 0) {
                // Error logging disabled for performance
                // if (logger) {
                //     logger->logError("Failed to add midpoint for edge " + std::to_string(i));
                // }
