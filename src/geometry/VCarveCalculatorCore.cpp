 ...
 *
 * Core V-carve calculation functionality - constructors and main generation methods.
...
VCarveResults VCarveCalculator::generateVCarvePaths(const MedialAxisResults& medialResults,
    ...
    try {
        ...
        // This preserves the exact vertices from OpenVoronoi without additional interpolation
        ...
        for (size_t i = 0; i < medialResults.chains.size(); ++i) {
            ...
            for (size_t j = 0; j < chain.size(); ++j) {
                // Calculate V-carve depth using the exact clearance radius from OpenVoronoi
                // clearances[j] is in cm, convert to mm for depth calculation
                double clearanceMm = clearances[j] * 10.0;
                double depth = calculateVCarveDepth(clearanceMm,
                                                  params.toolAngle,
                                                  params.maxVCarveDepth);

                // Create V-carve point - chain points are already in world coordinates (cm)
                // Convert to mm for consistency with the rest of the system
