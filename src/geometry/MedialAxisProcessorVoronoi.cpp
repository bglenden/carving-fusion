...
std::vector<SampledMedialPath> MedialAxisProcessor::getSampledPaths(
  ...
  // The chains in results are already in world coordinates (cm from
  // computeMedialAxisFromProfile) We just need to convert from cm to mm for the
  // sampling function
  std::vector<std::vector<Point2D>> worldChains;
  std::vector<std::vector<double>> worldClearances;

  log("Converting " + std::to_string(results.chains.size()) +
      " chains from world cm to world mm");
  ...
  for (size_t i = 0; i < results.chains.size(); ++i) {
    ...
    for (size_t j = 0; j < results.chains[i].size(); ++j) {
      ...
      Point2D worldPointMm(results.chains[i][j].x * 10.0,
                           results.chains[i][j].y * 10.0);
      ...
      if (i == 0 && j < 3) {
        log("Chain 0, point " + std::to_string(j) + ": world cm (" +
            std::to_string(results.chains[i][j].x) + ", " +
            std::to_string(results.chains[i][j].y) + ") -> world mm (" +
            std::to_string(worldPointMm.x) + ", " +
            std::to_string(worldPointMm.y) + "), clearance " +
            std::to_string(worldRadiusMm) + " mm");
      }
    }
...
bool MedialAxisProcessor::computeOpenVoronoi(
    const std::vector<Point2D>& transformedPolygon,
  ...
  try {
    ...
    log("Processing polygon with " + std::to_string(numSites) +
        " vertices, using " + std::to_string(bins) + " bins");

    // Insert point sites
    // Following Fusion convention: polygons are implicitly closed, no duplicate
    // vertices
    ...
    for (size_t i = 0; i < transformedPolygon.size(); ++i) {
      ...
      if (verbose_) {
        LOG_DEBUG("About to insert point " << i << ": (" << point.x << ", "
                                           << point.y << ")");
      }
      ...
      if (verbose_) {
        log("Added point " + std::to_string(id) + ": (" +
    ...
    log("Will insert " + std::to_string(numLines) +
        " line sites to form closed polygon");
    ...
    for (size_t i = 0; i < numLines; ++i) {
      ...
      if (verbose_) {
        LOG_DEBUG("About to insert line site " << i << ": " << startId << " -> "
                                               << endId);
      }

      log("Inserting line site " + std::to_string(i) + ": " +
      ...
      try {
        ...
        log("ERROR inserting line site " + std::to_string(i) +
            ": unknown exception");
        throw;
      }
    }
    ...
    ovd::MedialAxisWalk walker(
        graph, medialAxisWalkPoints_);  // Configurable intermediate points
    ovd::MedialChainList chainList = walker.walk();

    // REMOVED: Enhancement of linear edges - we want to preserve exact
    // OpenVoronoi vertices to avoid double interpolation. Fusion will handle
    // spline fitting through the raw vertices.
    ...
    for (const auto& chain : chainList) {
      ...
      for (const auto& pointList : chain) {
        for (const auto& medialPoint : pointList) {
          ...
          Point2D worldPoint =
              transformFromUnitCircle(unitPoint, results.transform);
          worldChain.push_back(worldPoint);

          // Convert clearance radius back to world scale
          double worldClearance =
              medialPoint.clearance_radius / results.transform.scale;
          worldClearances.push_back(worldClearance);
    ...
    results.errorMessage =
        "OpenVoronoi computation failed: " + std::string(e.what());
