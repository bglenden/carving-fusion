...
VCarvePath VCarveCalculator::convertSampledPath(
    const SampledMedialPath& sampledPath,
  ...
  for (const auto& sampledPoint : sampledPath.points) {
    ...
    // corners)
    double depth = calculateVCarveDepth(
        sampledPoint.clearanceRadius, params.toolAngle, params.maxVCarveDepth);

    // Always add points - even zero clearance points are important for corners
    VCarvePoint vcarvePoint(sampledPoint.position, depth,
...
std::vector<VCarvePath> VCarveCalculator::optimizePaths(
    const std::vector<VCarvePath>& paths,
    const Adapters::MedialAxisParameters& /* params */) {
  if (paths.empty()) {
    return paths;
  }
  ...
  std::sort(optimizedPaths.begin(), optimizedPaths.end(),
            [](const VCarvePath& a, const VCarvePath& b) {
...
bool VCarveCalculator::canConnectPaths(const VCarvePath& path1,
                                       const VCarvePath& path2,
                                       double tolerance) {
  ...
  double dist1 = std::sqrt(std::pow(p1_end.x - p2_start.x, 2) +
                           std::pow(p1_end.y - p2_start.y, 2));
  double dist2 = std::sqrt(std::pow(p1_end.x - p2_end.x, 2) +
                           std::pow(p1_end.y - p2_end.y, 2));
  double dist3 = std::sqrt(std::pow(p1_start.x - p2_start.x, 2) +
                           std::pow(p1_start.y - p2_start.y, 2));
  double dist4 = std::sqrt(std::pow(p1_start.x - p2_end.x, 2) +
                           std::pow(p1_start.y - p2_end.y, 2));

  return (dist1 <= tolerance || dist2 <= tolerance || dist3 <= tolerance ||
          dist4 <= tolerance);
}

VCarvePath VCarveCalculator::mergePaths(const VCarvePath& path1,
                                        const VCarvePath& path2) {
  VCarvePath merged;
  ...
  if (dist1 <= tolerance) {
    ...
    merged.points.insert(merged.points.end(), path2.points.begin(),
                         path2.points.end());
  ...
  double dist3 = std::sqrt(std::pow(p1_start.x - p2_end.x, 2) +
                           std::pow(p1_start.y - p2_end.y, 2));
  if (dist3 <= tolerance) {
    merged.points = path2.points;
    merged.points.insert(merged.points.end(), path1.points.begin(),
                         path1.points.end());
    merged.totalLength = merged.calculateLength();
    return merged;
  }
  ...
  double dist4 = std::sqrt(std::pow(p1_start.x - p2_start.x, 2) +
                           std::pow(p1_start.y - p2_start.y, 2));
  if (dist4 <= tolerance) {
    // Add path2 in reverse order
    for (auto it = path2.points.rbegin(); it != path2.points.rend(); ++it) {
