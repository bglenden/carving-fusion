...
ArcParams TriArc::getArcParameters(int arcIndex) const {
    ...
    if (isConcave) {
        // Use cross product to determine if we should sweep clockwise or counterclockwise
        ...
        // For concave arcs, we want the arc that curves inward toward triangle center
        // Since our center is positioned away from triangle center, we want the smaller arc
...
Point2D TriArc::getPerpendicularNormal(int arcIndex) const {
    ...
    // For CONCAVE arcs (negative bulge), choose normal that points AWAY from triangle centroid
    ...
    // For concave arcs, we want the normal that points AWAY from centroid (more negative dot
    // product)
    if (dot1 < dot2) {
...
Point2D TriArc::calculateArcCenter(const Point2D& p1, const Point2D& p2, double bulgeFactor,
                                   int arcIndex) const {
    ...
    // Use the getPerpendicularNormal method which already handles centroid direction
...
std::pair<double, double> TriArc::calculateArcAngles(const Point2D& center, const Point2D& p1,
...
// NOTE: Polygon vertices are now extracted directly from Fusion geometry using strokes
// This method has been removed because it incorrectly used original bulge factors
// instead of actual Fusion geometry. The polygonization is now handled by
// FusionAPIAdapter::extractProfileVertices() which uses actual geometric curves from Fusion.
