 ...
 * Extracts geometry as soon as profiles are selected to prevent stale token issues
 */
...
void GeneratePathsCommandHandler::extractAndCacheProfileGeometry(
    ...
    try {
        ...
        if (profileLoops) {
            for (size_t loopIdx = 0; loopIdx < profileLoops->count(); ++loopIdx) {
                ...
                for (size_t curveIdx = 0; curveIdx < profileCurves->count(); ++curveIdx) {
                    ...
                    if (worldGeometry) {
                        // Use tessellation like existing code to get proper polygon vertices
                        ...
                        if (evaluator) {
                            ...
                            if (evaluator->getParameterExtents(startParam, endParam)) {
                                ...
                                // For lines, use coarser tolerance since they don't need tessellation
                                if (auto line = sketchEntity->cast<adsk::fusion::SketchLine>()) {
                                    chordTolerance = 0.1;  // 1mm tolerance for lines (should only give 2 points)
                                } else {
                                    // For arcs, circles, splines etc., use fine tolerance for smooth curves
                                    chordTolerance = 0.005;  // 0.05mm tolerance for accurate curve representation
                                }
        ...
        LOG_INFO("Successfully cached geometry for profile " << index << " from sketch '" << profileGeom.sketchName 
                 << "' with " << vertices.size() << " vertices and area " << profileGeom.area << " sq cm");
