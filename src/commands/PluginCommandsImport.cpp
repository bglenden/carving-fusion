...
void ImportDesignCommandHandler::notify(
    ...
    try {
        ...
        inputs->addTextBoxCommandInput("titleText", "",
            "<b>Import Design</b><br/>Import a JSON chip carving design file with Leaf and TriArc shapes, then optionally select a construction plane or surface for placement.",
            3, true);
        ...
        planeSelection->tooltip("Optional: Select a construction plane or flat surface for the sketch. Must be parallel to XY plane. Defaults to XY plane if not selected.");
                        ...
                        if (stringInput) {
                            if (!selectedFilePath_.empty()) {
                                ...
                                std::string filename = (lastSlash != std::string::npos) ?
                                    selectedFilePath_.substr(lastSlash + 1) : selectedFilePath_;
