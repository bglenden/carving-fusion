...
void SettingsCommandHandler::createSettingsInputs(
    ...
    try {
        ...
        showInfoCheckbox->tooltip(
            "When enabled, displays detailed INFO and DEBUG log messages in the Text Commands window.\n"
        ...
        adsk::core::Ptr<adsk::core::TextBoxCommandInput> infoText =
            loggingInputs->addTextBoxCommandInput("loggingInfo", "",
                "Note: This setting applies immediately and persists for the current session only.",
                1, true);
    ...
    try {
        ...
        if (showInfoCheckbox) {
            if (showInfoCheckbox->value()) {
                ...
                LOG_INFO("Log level set to INFO (showing INFO, WARNING, and ERROR messages)");
#endif
            } else {
