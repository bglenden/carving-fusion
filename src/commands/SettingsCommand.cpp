...
void SettingsCommandHandler::notify(
  ...
  try {
    ...
    class ExecuteHandler : public adsk::core::CommandEventHandler {
     public:
      explicit ExecuteHandler(SettingsCommandHandler* parent)
          : parent_(parent) {}
...
void SettingsCommandHandler::createSettingsInputs(
  ...
  try {
    ...
    adsk::core::Ptr<adsk::core::CommandInputs> loggingInputs =
        loggingGroup->children();

    // Get current log level to set checkbox state
    LogLevel currentLevel = GetMinLogLevel();
    bool showInfoDebug =
        (currentLevel == LogLevel::INFO || currentLevel == LogLevel::LOG_DEBUG);

    // Add checkbox for INFO/DEBUG messages
    adsk::core::Ptr<adsk::core::BoolValueCommandInput> showInfoCheckbox =
        loggingInputs->addBoolValueInput("showInfoDebugMessages",
                                         "Show INFO and DEBUG messages", true,
                                         "", showInfoDebug);
    showInfoCheckbox->tooltip(
        "When enabled, displays detailed INFO and DEBUG log messages in the "
    ...
    adsk::core::Ptr<adsk::core::TextBoxCommandInput> infoText =
        loggingInputs->addTextBoxCommandInput(
            "loggingInfo", "",
...
void SettingsCommandHandler::applySettings(
    adsk::core::Ptr<adsk::core::CommandInputs> inputs) {
  try {
    // Get the checkbox value
    adsk::core::Ptr<adsk::core::BoolValueCommandInput> showInfoCheckbox =
        inputs->itemById("showInfoDebugMessages");
    ...
    if (showInfoCheckbox) {
      if (showInfoCheckbox->value()) {
        ...
        LOG_INFO(
            "Log level set to INFO (showing INFO, WARNING, and ERROR "
            "messages)");
#endif
      } else {
        ...
        LOG_WARNING(
            "Log level set to WARNING (INFO and DEBUG messages hidden)");
      }
    }
