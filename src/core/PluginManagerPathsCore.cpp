...
bool PluginManager::executeMedialAxisGeneration(const Adapters::SketchSelection& selection,
    ...
    try {
            ...
            try {
                ...
                if (results.success) {
                    ...
                    if (!results.chains.empty() && !results.chains[0].empty()) {
                        ...
                        // FIXED: Medial axis results are already in world coordinates (cm), don't convert again
        ...
        auto visualizationDuration = std::chrono::duration_cast<std::chrono::milliseconds>(visualizationEnd - visualizationStart);
        logger_->logInfo("⏱️ Construction geometry visualization took: " + std::to_string(visualizationDuration.count()) + "ms");
