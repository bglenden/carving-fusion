...
bool FusionSketch::addSpline3D(const std::vector<Geometry::Point3D>& points) {
  ...
  try {
    ...
    adsk::core::Ptr<adsk::fusion::SketchFittedSplines> splines =
        sketch_->sketchCurves()->sketchFittedSplines();
    if (splines) {
...
bool FusionSketch::addLine3D(double x1, double y1, double z1, double x2,
                             double y2, double z2) {
  if (!sketch_) {
    return false;
  }
  ...
  try {
    ...
    // Create the 3D line
    adsk::core::Ptr<adsk::fusion::SketchLines> lines =
...
bool FusionSketch::addPoint3D(double x, double y, double z) {
  ...
  try {
    ...
    adsk::core::Ptr<adsk::fusion::SketchPoints> points =
        sketch_->sketchPoints();
    if (points) {
...
std::vector<std::string> FusionSketch::getSketchCurveEntityIds() {
  ...
  try {
    ...
    adsk::core::Ptr<adsk::fusion::SketchCurves> sketchCurves =
        sketch_->sketchCurves();
    if (!sketchCurves) {
      return entityIds;
    }
    ...
    // Get all lines
    adsk::core::Ptr<adsk::fusion::SketchLines> lines =
        sketchCurves->sketchLines();
    if (lines) {
      for (size_t i = 0; i < lines->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchLine> line =
            lines->item(static_cast<int>(i));
    ...
    if (arcs) {
      for (size_t i = 0; i < arcs->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchArc> arc =
            arcs->item(static_cast<int>(i));
    ...
    // Get all splines
    adsk::core::Ptr<adsk::fusion::SketchFittedSplines> splines =
        sketchCurves->sketchFittedSplines();
    if (splines) {
      for (size_t i = 0; i < splines->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchFittedSpline> spline =
            splines->item(static_cast<int>(i));
    ...
    // Get all circles
    adsk::core::Ptr<adsk::fusion::SketchCircles> circles =
        sketchCurves->sketchCircles();
    if (circles) {
      for (size_t i = 0; i < circles->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchCircle> circle =
            circles->item(static_cast<int>(i));
    ...
    // Get all ellipses
    adsk::core::Ptr<adsk::fusion::SketchEllipses> ellipses =
        sketchCurves->sketchEllipses();
    if (ellipses) {
      for (size_t i = 0; i < ellipses->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchEllipse> ellipse =
            ellipses->item(static_cast<int>(i));
    ...
    adsk::core::Ptr<adsk::fusion::SketchEllipticalArcs> ellipticalArcs =
        sketchCurves->sketchEllipticalArcs();
    if (ellipticalArcs) {
      for (size_t i = 0; i < ellipticalArcs->count(); ++i) {
        adsk::core::Ptr<adsk::fusion::SketchEllipticalArc> ellipticalArc =
            ellipticalArcs->item(static_cast<int>(i));
    ...
    std::cout << "Error getting sketch curve entity IDs: " << e.what()
              << std::endl;
  } catch (...) {
