...
bool FusionWorkspace::extractCurvesFromProfile(
                    ...
                    if (plane) {
                        ...
                        if (normal) {
                            ...
                            if (std::abs(normal->z()) > 0.99) {
                                if (normal->z() > 0) {
                                    LOG_DEBUG("Sketch plane is correctly oriented (parallel to XY plane)");
                                } else {
    ...
    for (size_t i = 0; i < profileCurves->count(); ++i) {
        ...
        LOG_DEBUG("Curve " << i << " start: (" << curveData.startPoint->x() << ", " << curveData.startPoint->y() << ", " << curveData.startPoint->z() << ")");
        LOG_DEBUG("Curve " << i << " end: (" << curveData.endPoint->x() << ", " << curveData.endPoint->y() << ", " << curveData.endPoint->z() << ")");
