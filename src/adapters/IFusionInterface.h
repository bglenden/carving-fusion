...
struct MedialAxisParameters {
  ...
  double clearanceCircleSpacing =
      5.0;                 // Distance between clearance circles (mm)
  double crossSize = 3.0;  // Size of center cross marks in mm (0 = no crosses)
  bool forceBoundaryIntersections =
      true;  // Force sampling at boundary intersections
  bool showMedialLines =
      true;  // Show medial axis lines in construction geometry
  bool showClearanceCircles =
      true;  // Show clearance circles in construction geometry
  bool showPolygonizedShape = false;  // Show polygonized shape outline
  bool generateVisualization =
      false;  // Generate visualization sketches (default off)
  ...
  bool generateVCarveToolpaths =
      false;  // Generate V-carve toolpaths (default off)
  double maxVCarveDepth =
      25.0;  // Maximum V-carve depth in mm (safety limit, default 25mm)
...
class IUserInterface {
 public:
  ...
  virtual std::string showFileDialog(const std::string& title,
  ...
  virtual bool showParameterDialog(const std::string& title,
                                   MedialAxisParameters& params) = 0;
  virtual SketchSelection showSketchSelectionDialog(
...
class ISketch {
 public:
  ...
  virtual void addShape(const Geometry::Shape* shape,
                        ILogger* logger = nullptr) = 0;
  virtual std::string getName() const = 0;
  virtual bool addLineToSketch(double x1, double y1, double x2, double y2) = 0;
  virtual bool addArcToSketch(double centerX, double centerY, double radius,
                              double startAngle, double endAngle) = 0;
  virtual int addPointToSketch(double x, double y) = 0;
  virtual bool addArcByThreePointsToSketch(int startPointIndex,
                                           int midPointIndex,
                                           int endPointIndex) = 0;
  virtual bool addLineByTwoPointsToSketch(int startPointIndex,
                                          int endPointIndex) = 0;
  virtual bool deleteSketchPoint(int pointIndex) = 0;
  virtual void finishSketch() = 0;

  // Construction geometry methods
  virtual bool addConstructionLine(double x1, double y1, double x2,
                                   double y2) = 0;
  virtual bool addConstructionCircle(double centerX, double centerY,
                                     double radius) = 0;
  ...
  virtual bool addLine3D(double x1, double y1, double z1, double x2, double y2,
                         double z2) = 0;
...
class IWorkspace {
 public:
  ...
  virtual std::unique_ptr<ISketch> createSketchOnPlane(
      const std::string& name, const std::string& planeEntityId) = 0;

  // Create sketch in component that contains the target surface (enhanced for
  // cross-component organization)
  virtual std::unique_ptr<ISketch> createSketchInTargetComponent(
      const std::string& name, const std::string& surfaceEntityId) = 0;
  ...
  virtual bool extractProfileVertices(
  ...
  virtual std::string extractPlaneEntityIdFromProfile(
      const std::string& profileEntityId) = 0;
