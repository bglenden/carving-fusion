 ...
 *
 * Core functionality for FusionSketch - constructor, basic sketch operations, 2D geometry
...
bool FusionSketch::addArcToSketch(double centerX, double centerY, double radius, double startAngle,
                                 double endAngle) {
    if (!sketch_) {
        return false;
    }
...
bool FusionSketch::addArcByThreePointsToSketch(int startPointIndex, int midPointIndex,
                                              int endPointIndex) {
    if (!sketch_) {
        return false;
    }
    ...
    try {
        ...
        // startPoint: SketchPoint (for constraint), point: Point3D (geometry), endPoint: SketchPoint (for constraint)
