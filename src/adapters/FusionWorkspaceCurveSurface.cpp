...
double FusionWorkspace::getSurfaceZAtXY(const std::string& /*surfaceId*/,
                                        double x, double y) {
  ...
  try {
    ...
    LOG_DEBUG(
        "Using enhanced universal ray casting for cross-component surface "
        "detection");

    // Create a ray from above the surface pointing downward (in world
    // coordinates)
    double rayStartZ =
        1000.0;  // Start ray 10 meters above (should be well above any surface)
    Ptr<adsk::core::Point3D> rayOrigin =
        adsk::core::Point3D::create(x, y, rayStartZ);
    Ptr<adsk::core::Vector3D> rayDirection =
        adsk::core::Vector3D::create(0.0, 0.0, -1.0);  // Pointing down
    ...
    if (occurrences) {
      LOG_DEBUG("Found " << occurrences->count()
                         << " component occurrences to search");
    ...
    LOG_DEBUG("Searching " << allComponents.size()
                           << " total components for surfaces");
    ...
    for (size_t compIdx = 0; compIdx < allComponents.size(); ++compIdx) {
      ...
      Ptr<adsk::core::ObjectCollection> hitPoints =
      ...
      try {
        ...
        if (intersectedEntities && intersectedEntities->count() > 0) {
          LOG_DEBUG("Component " << compIdx << " ray casting found "
                                 << intersectedEntities->count()
                                 << " intersected entities with "
          ...
          for (size_t i = 0; i < hitPoints->count(); ++i) {
            ...
            if (hitPoint) {
              ...
              if (hitZ > bestZ) {
                ...
                LOG_DEBUG("New topmost surface found at Z = "
                          << bestZ << " cm in component " << compIdx);
              }
            }
          }
        }
          ...
          for (size_t meshIdx = 0; meshIdx < meshBodies->count(); ++meshIdx) {
            ...
            for (size_t triIdx = 0; triIdx < nodeIndices.size(); triIdx += 3) {
              ...
              if (hitZ > bestZ && hitZ < rayStartZ) {
                bestZ = hitZ;
                foundValidPoint = true;
                LOG_DEBUG("Mesh intersection found at Z = "
                          << bestZ << " cm in component " << compIdx
                          << ", mesh " << meshIdx);
              }
            }
        ...
        LOG_DEBUG("Component " << compIdx
                               << " ray casting failed: " << e.what());
        // Continue with other components
      } catch (...) {
        LOG_DEBUG("Component " << compIdx
                               << " ray casting failed with unknown exception");
        // Continue with other components
      }
    }

    if (foundValidPoint) {
      LOG_DEBUG("Enhanced ray casting found topmost surface at Z = "
                << bestZ << " cm across " << allComponents.size()
                << " components");
      return bestZ;
    } else {
      LOG_WARNING("Enhanced ray casting found no valid surface at XY location ("
                  << x << ", " << y << ") across " << allComponents.size()
                  << " components");
      return std::numeric_limits<double>::quiet_NaN();
    }
  } catch (const std::exception& e) {
